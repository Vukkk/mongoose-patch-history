...
exports.default = function (schema, opts) {
  ...
  schema.pre('findOneAndReplace', function (next) {
    ...
    this.model.findOne(this.getQuery()).then(function (original) {
      if (original) _this2._originalId = original._id;
      original = original || new _this2.model({});
      _this2._original = toJSON(original.data());

      var ops = _fastJsonPatch2.default.compare(original.isNew ? {} : toJSON(_this2._original) || {}, toJSON(new _this2.model(_this2.getUpdate()).data()));
      ...
      if (options.trackOriginalValue) {
        ops.map(function (entry) {
          var path = (0, _lodash.tail)(entry.path.split('/')).join('.');
          entry.originalValue = (0, _lodash.get)(original.isNew ? {} : original._original, path);
        });
      }

      // assemble patch data
      var data = { ops: ops, ref: _this2._originalId };
      (0, _lodash.each)(options.includes, function (type, name) {
        data[name] = original[type.from || name];
      });

      return original.patches.create(data).then(function () {
        return next();
      }, next);
    });
  });
  ...
  function preUpdateOne(next) {
    ...
    this.model.findOne(this._conditions).then(function (original) {
      if (original) _this3._originalId = original._id;
      original = original || new _this3.model({});
      _this3._original = toJSON(original.data());
    }).then(function () {
      return next();
    }).catch(next);
  }
  ...
  function postUpdateOne(result, next) {
    ...
    this.model.findOne(conditions).then(function (doc) {
      if (!doc) return next();
      doc._original = _this4._original;
  ...
  function preUpdateMany(next) {
    ...
    this.model.find(this._conditions).then(function (originals) {
      ...
      _this5._originalIds = originalIds;
      _this5._originals = originalData;
    }).then(function () {
      return next();
    }).catch(next);
  }
  ...
  function postUpdateMany(result, next) {
    ...
    this.model.find(conditions).then(function (docs) {
      return _bluebird2.default.all(docs.map(function (doc, i) {
        doc._original = _this6._originals[i];
        return createPatch(doc, _this6.options);
      }));
    }).then(function () {
      return next();
    }).catch(next);
